...
const Questions = ({questions}) => {
    ...
    const generateQuiz = (array, parentId) => {
        const generate = array.map(quest => {
            return <button key={nanoid()} name={parentId} value={quest.option.name} onClick={selectAnswer} className={`${quest.option.isHeld}`}>{quest.option.name}</button>
        })
        return generate
    }
